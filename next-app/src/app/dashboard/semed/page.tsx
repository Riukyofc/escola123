...
import { dbGetAll, dbGetAllEscola, getConfig, getEscolaAtiva, getEscolaInfo } from '@/lib/data';
...
export default function SemedInicio() {
  ...
  const isGlobal = !escolaAtiva || escolaAtiva === 'semed';
  const schoolInfo = getEscolaInfo();
  const cfg = getConfig();

  if (!session) return null;

  // ─── DATA RESOLVER (GLOBAL VS LOCAL) ───
  const schools = dbGetAll<Record<string, any>>('escolas_rede');
  const turmas = dbGetAllEscola<Record<string, any>>('turmas').filter(t => t.ativo);
  const alunos = dbGetAllEscola<Record<string, any>>('alunos').filter(a => a.ativo);
  const profs = dbGetAllEscola<Record<string, any>>('professores').filter(p => p.ativo);
  const discs = dbGetAll<Record<string, any>>('disciplinas').filter(d => d.ativo);
  const notas = dbGetAllEscola<Record<string, any>>('notas');
  const freq = dbGetAllEscola<Record<string, any>>('frequencia');
  const repasses = dbGetAll<Record<string, any>>('repasses_financeiros');
  ...
  // ─── Financial calculations (SEMED only) ───
  const totalRepasses = repasses.reduce((s, r) => s + (r.escolaId === escolaAtiva || isGlobal ? (Number(r.valor) || 0) : 0), 0);
  ...
  if (isGlobal) {
      ...
      <PageTransition>
        <WelcomeBanner
          ...
          stats={[
            ...
            { label: 'Alunos na Rede', value: alunos.length },
            { label: 'Professores', value: profs.length },
            { label: 'Freq. Média', value: `${avgFreq}%` },
          ]}
        />
        ...
        <div className="stats-row stagger-container" style={{ marginBottom: 20 }}>
          ...
          <StatCard label="Escolas Cadastradas" value={schools.length} icon="fa-school" color="red" />
          ...
          <motion.div style={{ marginBottom: 20 }} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div className="panel-card">
              <div className="panel-card-header">
                <div className="panel-card-title"><i className="fa-solid fa-triangle-exclamation" /> Alertas Gerais da Rede</div>
    ...
    // ─── RENDERING SCHOOL-SPECIFIC VIEW (DIRECTOR EQUIVALENT) ───
    const schoolBarData = {
    ...
    return (
      <PageTransition>
        <WelcomeBanner
          tag="Gestão da Escola Ativa" tagIcon="fa-school"
          name={schoolInfo.nomeAbreviado || schoolInfo.nome}
          sub={`Visão de Direção da Escola · Ano Letivo ${schoolInfo.anoLetivo}`}
          avatar="E"
          gradient="linear-gradient(135deg,#0f2347 0%,#1d3a74 100%)"
          stats={[
            { label: 'Alunos', value: alunos.length },
            { label: 'Turmas', value: turmas.length },
            { label: 'Professores', value: profs.length },
            { label: 'Aprov. Geral', value: `${taxa}%` },
          ]}
        />
        ...
        <div className="stats-row stagger-container" style={{ marginBottom: 28 }}>
          ...
          <StatCard label="Freq. Média Escola" value={`${avgFreq}%`} icon="fa-calendar-check" color="amber" />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '20px', marginBottom: 20 }}>
          <motion.div style={chartCard} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <div style={{ height: '280px' }}>
              <Bar data={schoolBarData} options={{
                maintainAspectRatio: false,
                plugins: {
        ...
        <motion.div style={chartCard} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          ...
          <div style={{ height: '260px' }}>
            <Line data={schoolLineData} options={{
              maintainAspectRatio: false,
              plugins: {
                legend: { display: false },
                tooltip: {
